...
#     nq = torch.sum(label.data==1).item() # number of tuples 32
#     S = x.size(1) // nq # number of images per tuple including query: 1+1+n
#
...
def triplet_loss(anchor, pos, neg, margin=0.1):
    nq = anchor.shape[0]

    dist_pos = torch.sum(torch.pow(anchor - pos, 2), dim=0)
    dist_neg = torch.sum(torch.pow(anchor - neg, 2), dim=0)

    return torch.sum(torch.clamp(dist_pos - dist_neg + margin, min=0)) / nq


def sos_loss(anchor, pos, neg):
    # nq = torch.sum(anchor.data == -1).item()
    #anchor.shape = [32,2048]

    nq = anchor.shape[0]
    dist_an = torch.sum(torch.pow(anchor - neg, 2), dim=0)
    dist_pn = torch.sum(torch.pow(pos - neg, 2), dim=0)
