...
class SOLAR_Global_Retrieval(nn.Module):
    def __init__(self, architecture, features, lwhiten, pool, whiten, meta, mode='train', pretrained_type='gl18',
            soa_layers='45'):
        ...
        self.classifier = nn.Sequential(nn.Linear(512, 5), nn.Sigmoid())

    def forward(self, x):
        # x -> features #x = [32,3,224,224]
        o = self.features(x, self.mode) # o = [32,2048,7,7]
        ...
        if self.lwhiten is not None:
            # o = self.norm(o)
            s = o.size()
            o = o.permute(0, 2, 3, 1).contiguous().view(-1, s[1]) #o = [1568,2048]
            o = self.lwhiten(o) #o = [1568,2048]
            o = o.view(s[0], s[2], s[3], self.lwhiten.out_features).permute(0, 3, 1, 2)
            # o = self.norm(o)

        # features -> pool -> norm
        o = self.norm(self.pool(o)).squeeze(-1).squeeze(-1) #o = [32,2048]

        # if whiten exist: pooled features -> whiten -> norm
        if self.whiten is not None:
            o = self.norm(self.whiten(o)) #o = [32,2048]
...
def init_network(params):
    ...
    local_whitening = params.get('local_whitening', True)
    pooling = params.get('pooling', 'gem')
    p = params.get('p', 3.)
    regional = params.get('regional', True)
    whitening = params.get('whitening', True)
    mean = params.get('mean', [0.485, 0.456, 0.406])
    std = params.get('std', [0.229, 0.224, 0.225])
    pretrained = params.get('pretrained', False)
    soa = params.get('soa', True)
    ...
    net = SOLAR_Global_Retrieval(architecture, features, lwhiten, pool, whiten, meta, pretrained_type=pretrained_type,
                                 soa_layers=soa_layers, mode=mode)

    return net


def extract_vectors(net, images, image_size, transform, bbxs=None, ms=[1], msp=1, print_freq=10, summary=None,
        mode='test'):
