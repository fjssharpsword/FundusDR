...
parser.add_argument('--model', type=str, default='GeM', help='GeM')
...
def Test():
    ...
    for topk in [5, 10, 20, 50]:
        ...
        for i in range(N_CLASSES):
            logger.info('GeM mHR of {} is {:.4f}'.format(CLASS_NAMES[i], np.mean(mHRs[i])))
        logger.info("GeM Average mHR@{}={:.4f}".format(topk, np.mean(mHRs_avg)))
        # average precision
        for i in range(N_CLASSES):
            logger.info('GeM mAP of {} is {:.4f}'.format(CLASS_NAMES[i], np.mean(mAPs[i])))
        logger.info("GeM Average mAP@{}={:.4f}".format(topk, np.mean(mAPs_avg)))
