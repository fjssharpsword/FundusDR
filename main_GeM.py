...
def Train():
    print('********************load data********************')
    dataloader_train = get_train_dataloader(batch_size=16, shuffle=True, num_workers=8)
    ...
    for epoch in range(config['MAX_EPOCHS']):
        ...
        with torch.autograd.enable_grad():
            for batch_idx, (image, label) in enumerate(dataloader_train):
                ...
                #backward
                loss_tensor = triplet_loss.forward(var_feat_anchor, var_feat_pos, var_feat_neg)
