...
def main():
    ...
    if args.soa:
        ...
        if args.unfreeze_last:
            parameters.append(
                {'params': model.features.conv5_x.parameters(), 'lr': args.lr * 0.0})  # , 'weight_decay': 0})
...
def train(train_loader, model, criterion, criterionB, optimizer, epoch, summary):
    ...
    for i, (_input, target) in enumerate(train_loader):
        ...
        if (i + 1) % args.print_freq == 0 or i == 0 or (i + 1) == len(train_loader):
            ...
            summary.add_scalar('train/1st-order_loss', loss_A.item(),
                               global_step=epoch * args.query_size + i * args.batch_size)
            if args.sos:
                summary.add_scalar('train/2nd-order_loss', loss_B.item(),
                                   global_step=epoch * args.query_size + i * args.batch_size)
...
def test(datasets, net, epoch, summary):
    ...
    for dataset in datasets:
        ...
        vecs = extract_vectors(net, images, image_size, transform, summary=summary, mode='test',
                               ms=ms)  # implemented with torch.no_grad

        print('>> {}: query images...'.format(dataset))
        qvecs = extract_vectors(net, qimages, image_size, transform, bbxs, summary=summary, mode='test',
                                ms=ms)  # implemented with torch.no_grad
